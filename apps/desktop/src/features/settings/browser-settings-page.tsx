				...
				<SectionBlock title="CloakBrowser Installation">
					...
					<div className="space-y-2 rounded-lg border border-border/50 bg-surface p-4 text-sm">
						...
						<p className="text-muted-foreground text-xs">
							Use native Windows PowerShell for CloakBrowser on Windows, not
							WSL.
						</p>
					</div>
								...
								{(discoveredQuery.data ?? []).map((item) => (
									...
									>
										...
										<Button
											...
											className="shrink-0"
											disabled={
												!desktop || !item.valid || setExecutable.isPending
											}
...
function CapabilityRow({
	...
	return (
		<div className="flex justify-between text-sm">
			...
			<span
				className={supported ? "text-emerald-400" : "text-muted-foreground"}
			>
